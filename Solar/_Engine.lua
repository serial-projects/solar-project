...
function module.Sol_LoadSettings(engine)
  local settings_loaded=SCF.SCF_LoadFile(SS_Path.Sol_MergePath({engine.root, "settings.slr"}))
  --> section "window"
...
function module.Sol_KeypressEventEngine(engine, key)
  if      engine.current_mode == SV_Consts.engine_modes.MENU   then
    mwarn("engine.current_mode is not yet implemented.")
  elseif  engine.current_mode == SV_Consts.engine_modes.WORLD  then
    SWM_Mode.Sol_KeypressEventWorldMode(engine, engine.world_mode, key)
  else
    mwarn("engine.current_mode is not yet implemented.")
  end
end
...
function module.Sol_TickEngine(engine)
  ...
  if      engine.current_mode == SV_Consts.engine_modes.MENU   then
    mwarn("engine.current_mode is not yet implemented.")
  elseif  engine.current_mode == SV_Consts.engine_modes.WORLD  then
    SWM_Mode.Sol_TickWorldMode(engine, engine.world_mode)
  else
    mwarn("engine.current_mode is not yet implemented.")
  end
end
