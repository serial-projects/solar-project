...
function module.SPI_SetDataToInstance(context, instance, token, value)
    ...
    local   performing_table = {
        ...
        ["!"]=function()
            ...
            else instance:set_error("no register %s exists.", string.upper(without_prefix)) end
        end
    }
    local do_perform = performing_table[possible_prefix]
    if do_perform then do_perform()
    else instance:set_error("unable to set \"%s\" to temporary value: \"%s\"", value, token) end
end
